...
namespace clipper {
	...
	TriState isPolyHole(TPolyPt *pt)
	{
		TPolyPt *bottomPt = pt;
		TPolyPt *ptStart = pt;
		pt = pt->next;
		...
		return bottomPt->isHole;
	}

	bool ValidateOrientation(TPolyPt *pt)
	{
		TriState isHole = isPolyHole(pt);
		double area = GetArea(pt);
		bool IsClockwise = (area > 0);
		bool hole = (isHole == sTrue);
		bool ok = (IsClockwise != hole);
		return ok;
	}
	//------------------------------------------------------------------------------
	...
	{
		AddPolyPt( e1, pt );
		//e2->outIdx = e1->outIdx; /////////////// move this below
		...
		e2->outIdx = e1->outIdx; //////////// moved to here
